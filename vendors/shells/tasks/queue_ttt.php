...
class QueueTttTask extends Shell {
	/**
	 ...
	 */
	public function add() {
		$this->out('Adding one "TTT" to the Queue.');
		/**
		 * Adding a task of type 'helloworld' with no additionally passed data
		 */
		if ($this->QueuedTask->createJob('ttt', null)) {
			$this->out('OK, TTT job created, now run the worker');
		} else {
